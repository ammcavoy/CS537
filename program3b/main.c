...
int main(int argc, char* argv[]){
	...
	CommandLine *inputs = parseCommandLine(argc, argv, BUFFER_SIZE);
	...
	char **orderedArray = getOrdering(graph);
	...
	if(argc == 1){
		for(int i = 0; i < graph->numInOrderedArray; i++){
			...
			if(isNotDependencyOfClean(temp)){
				if(needToAddNode(temp) && (strcmp(temp->name,clean))){
					buildProc(temp->commands, temp->numCommands,BUFFER_SIZE);
		...
		for(int m = 0; m < inputs->numProcesses; m++){
			procs = findNode(graph, inputs->process[m]);
			if(procs == NULL){
				fprintf(stderr, "Unable to find build process for %s.\n", inputs->process[m]);
				exit(-1);
			}
			...
			for(int k = 0; k < graph->size; k++){
				if(isRelative(graph, procs, orderedArray[k])){
			...
			if(needToAddNode(procs)){
				buildProc(procs->commands, procs->numCommands,BUFFER_SIZE);
			}else{
				fprintf(stderr, "'%s' is up to date.\n",procs->name);
			}
		}
