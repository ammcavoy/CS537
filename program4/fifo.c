...
typedef struct fifonode{  //the struct that will be used to determine what page is removed when mem is full
	char * pid;
	char * vpn;
...
int main(int argc, char *argv[]){
	params* input = parseCMDLine(argc, argv);  //takes in the parameters from the command line
	...
	tracefile = fopen(input->filename, "r");  //open the trace file 
	...
	//first pass through the trace file

	while(!feof(tracefile)){
		line_count += 1;
		pos = ftell(tracefile);
		fscanf(tracefile, "%s %s", proc_id, vpn);
		if(strcmp(proc_id, current->pid) == 0){	  //if the current linked node is for the correspoding line in the trace file 
			if(pos - current->end > 15){
				...
				current->starts[current->start_count] = pos;			//update the values for node
				current->start_count += 1;
			}
			current->end = pos; 
		}
	...
	fifonode * fifohead = NULL;
	fifonode * fifotail = NULL;
		...
		if(execute != NULL){
			...
			if(strcmp(execute->pid, proc_id) != 0){
				...
				fscanf(tracefile, "%s %s", proc_id, vpn);
					...
					fifonode * fifocurr = fifohead;
					fifonode * fifoparent = NULL;
					while(fifocurr != NULL){
						if(strcmp(proc_id, fifocurr->pid) == 0){
							if(fifoparent == NULL){
							       	fifohead = fifohead->next;
								fifocurr = fifohead;
							}	
							else{
								fifoparent->next = fifocurr->next;
								fifocurr = fifocurr->next;
							}
						}
						else{
							fifoparent = fifocurr;
			...
			p_copy = malloc(sizeof(proc_id) + 1);
                        ...
                        if(fifohead == NULL){
                        	fifohead = (fifonode *)malloc(sizeof(fifonode));
                                fifotail = fifohead;
                                fifohead->next = NULL;
                                fifohead->pid = p_copy;
                                fifohead->vpn = v_copy;
                        	...
                        	fifotail->next = (fifonode *)malloc(sizeof(fifonode));
